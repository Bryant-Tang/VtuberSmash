...
class ReadRuleScene(Scene):
    ...
    def start_showing(self):
        ThreadManager().run_func_as_new_thread(target=self._show_rule_thread_func)

    def _show_rule_thread_func(self):
        self.clear_component()
        self._page_list = [Component(image.get_image(image.GAME_RULE_0)),
                           Component(image.get_image(image.GAME_RULE_1)),
                           Component(image.get_image(image.GAME_RULE_2)),
                           Component(image.get_image(image.GAME_RULE_3)),
                           Component(image.get_image(image.GAME_RULE_4)),]
